...
class VRLVBlockBase(nn.Module):
    ...
    def forward_ntc(self, feature,y,lmb_embedding):
        y_sample = y + torch.empty_like(y).uniform_(-0.5, 0.5)
        feature_next_ntc = self.resnet_end(feature + self.z_proj(y_sample) , lmb_embedding)
        return feature_next_ntc


    def forward_hjscc(self, feature, initial_image, scale_h,scale_w, enc_feature,lmb_embedding,snr):
        feature, miu, sigma = self.transform_prior(feature, lmb_embedding)
        y = self.transform_posterior(feature, enc_feature, lmb_embedding)
        y_sample = y + torch.empty_like(y).uniform_(-0.5, 0.5)  # ntc对应zi
        log_prob = gaussian_log_prob_mass(miu, sigma, x=y_sample, bin_size=1.0, prob_clamp=1e-6)
        kl = -1.0 * log_prob
        prob = self.feature_probs_based_Gaussian(y_sample, miu, sigma)
        z_hat, cbr_z = self.HJSCC_EncDec(y, prob, initial_image, scale_h, scale_w,snr)
        feature_next_hjscc = self.resnet_end(feature + self.z_proj(z_hat),lmb_embedding)
        return feature_next_hjscc, y, kl,cbr_z
