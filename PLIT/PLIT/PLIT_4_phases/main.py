...
def train_one_epoch(epoch, net, train_loader, optimizer_G, device, logger):
    global global_step
    net.train()
    elapsed, losses, psnrs, psnr_jsccs_1,psnr_jsccs_2,psnr_jsccs_3,psnr_jsccs_4, bpps, cbrs, kls = [AverageMeter() for _ in range(10)]
    metrics = [elapsed, losses, psnrs, psnr_jsccs_1,psnr_jsccs_2,psnr_jsccs_3,psnr_jsccs_4, bpps, cbrs, kls]
    for batch_idx, input_image in enumerate(train_loader):
        ...
        global_step += 1
        loss,psnr_ntc,psnr_plit_1,psnr_plit_2,psnr_plit_3,psnr_plit_4,kl,cbr_z,bpp = net(input_image)
        ...
        psnr_jsccs_3.update(psnr_plit_3)
        psnr_jsccs_4.update(psnr_plit_4)
        psnrs.update(psnr)
        ...
        if (global_step % config.print_step) == 0 or global_step==1:
            ...
            log = (' | '.join([
                ...
                f'PSNR_JSCC_3 {psnr_jsccs_3.val:.2f} ({psnr_jsccs_3.avg:.2f})',
                f'PSNR_JSCC_4 {psnr_jsccs_4.val:.2f} ({psnr_jsccs_4.avg:.2f})',
...
def test(net, test_loader, logger):
    with torch.no_grad():
        ...
        for batch_idx, input_image in enumerate(test_loader):
            input_image = input_image.cuda()
            start_time = time.time()
            loss,psnr_ntc,psnr_plit_1,psnr_plit_2,psnr_plit_3,psnr_plit_4,kl,cbr_z,bpp = net(input_image,lmb=128)
            psnr = psnr_ntc
            cbrs.update(cbr_z)
            kls.update(kl.item())
            elapsed.update(time.time() - start_time)
    ...
    logger.info(f'Finish test! Average PSNR={psnr_jsccs_4.avg:.4f}dB,CBR={cbrs.avg:.4f}, CBR_all={cbrs.avg + cbr_sideinfo:.4f}')
    return losses.avg
...
def parse_args(argv):
    ...
    parser.add_argument("--checkpoint",
                        default='PLIT_4_phases_quality_1',
