...
class AF_BLC(nn.Module):
    ...
    def _snr_scaling(self, snr: torch.Tensor):
        snr_input = torch.log(snr) * 64 / math.log(8192)
        return snr_input

    def _get_snr_embedding(self, snr, n):
        snr = self.expand_to_tensor(snr, n=n)
        scaled = self._snr_scaling(snr)
        embedding = sinusoidal_embedding(scaled, dim=256, max_period=64)
        embedding = self.snr_embedding(embedding)
        return embedding

    def forward(self, x, snr, H, W):
        B, L, C = x.size()
        low, high = [16, 1024]  # original lmb space, 16 to 1024
        p = 3.0
        low, high = math.pow(low, 1 / p), math.pow(high, 1 / p)  # transformed space
        transformed_snr = low + (high - low) * ((torch.ones(B).cuda()) * snr / 20)
        transformed_snr = transformed_snr.cuda()
        snr = torch.pow(transformed_snr, exponent=p)
        e_snr = self._get_snr_embedding(snr, n=B)
        ...
        x = self.norm(x)
        # AdaLN
        embedding = self.embedding_layer(e_snr)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_windows = self.attn(x_windows,add_token=False,mask=self.attn_mask)  # nW*B, window_size*window_size, C
...
class BasicLayerDec(nn.Module):
    ...
    def __init__(self, dim, out_dim, input_resolution, depth, num_heads, window_size,
        ...
        if upsample is not None:
            ...
            self.upsample = None
        self.AF_BLC = AF_BLC(self.dim)
    def forward(self, x,snr,H,W):
        for _, blk in enumerate(self.blocks):
            x = self.AF_BLC(x,snr,H,W)
            x = blk(x)
            x = self.AF_BLC(x,snr,H,W)
