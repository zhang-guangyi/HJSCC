...
class VRLVBlockBase(nn.Module):
    ...
    def forward_ntc(self, feature, lmb_embedding, enc_feature=None):
        ...
        z_sample = qm + torch.empty_like(qm).uniform_(-0.5, 0.5)  # ntc对应zi
    ...
    def forward_plit(self, feature, z_hat,cbr_z,lmb_embedding,block,threshold):
        node=[3,5,7,9]
        feature, pm, pv = self.transform_prior(feature, lmb_embedding)
        if threshold > node[block-1]:
            z_hat=pm
            cbr_z=0
        feature_next_plit=self.resnet_end(feature + self.z_proj(z_hat) , lmb_embedding)
        return feature_next_plit, cbr_z
