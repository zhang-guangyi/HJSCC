...
class RateAdaptionDecoder(nn.Module):
    ...
    def forward(self, x, indexes):
        ...
        x_BLC = torch.matmul(x_BLC.unsqueeze(2), w).squeeze(2) + b  # BLN
        out = x_BLC.reshape(B, H, W, -1).permute(0, 3, 1, 2)
        return out


class JSCCDecoder(nn.Module):
    def __init__(self, zdim, embed_dim=256, depths=[1, 1, 1],
        ...
        for i_layer in range(len(depths)):
            layer = BasicLayerDec(dim=zdim, out_dim=zdim, input_resolution=input_resolution,
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(zdim, 4 * zdim, kernel_size=5, stride=1, padding=2, bias=True),
            nn.GELU(),
            nn.Conv2d(4 * zdim, zdim, kernel_size=5, stride=1, padding=2, bias=True)
        )
    def forward(self, x, indexes,snr):
        x = x + self.conv(x)
        B, _, H, W = x.size()
