...
class Channel(nn.Module):
    ...
    def forward(self, input, snr=20,avg_pwr=None, power=1):
        flag=0
        if avg_pwr is None:
        ...
        if torch.numel(channel_in)%2!=0:
            flag=1
            add=torch.zeros([1]).to(self.device)
            channel_in=torch.cat((channel_in,add),dim=0)
        channel_shape=channel_in
        ...
        channel_rx = torch.zeros_like(channel_shape.reshape(-1))
        channel_rx[::2] = torch.real(channel_output)
        channel_rx[1::2] = torch.imag(channel_output)
        if torch.numel(channel_rx)!=torch.numel(channel_tx):
            channel_rx=channel_rx[0:-1]
