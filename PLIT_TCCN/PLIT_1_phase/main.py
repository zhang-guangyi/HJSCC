...
def train_one_epoch(epoch, net, train_loader, optimizer_G, device, logger):
    ...
    for batch_idx, input_image in enumerate(train_loader):
        ...
        global_step += 1
        loss,psnr_ntc,psnr_plit,kl,cbr_z,bpp = net(input_image)
...
def test(net, test_loader, logger):
    with torch.no_grad():
        ...
        for batch_idx, input_image in enumerate(test_loader):
            input_image = input_image.cuda()
            start_time = time.time()
            loss,psnr_ntc,psnr_plit,kl,cbr_z,bpp = net(input_image,lmb=128)
            psnr = psnr_ntc
            cbrs.update(cbr_z)
            kls.update(kl.item())
            elapsed.update(time.time() - start_time)
...
def parse_args(argv):
    ...
    parser.add_argument("--checkpoint",
                        default='PLIT_1_phase_quality_1',
